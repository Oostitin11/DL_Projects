...
# Ambil ukuran frame untuk video writer
    ...
    for (object_id, centroid) in objects.items():
        ...
        if object_id in previous_x:
            ...
            if prev_x > line_position and current_x < line_position:
                left_count += 1
                print(f"Person {object_id} moved LEFT (keluar).")
            elif prev_x < line_position and current_x > line_position:
                right_count += 1
                print(f"Person {object_id} moved RIGHT (masuk).")
        previous_x[object_id] = current_x

    cv2.line(frame, (line_position, 0), (line_position, h), (0, 0, 255), 2)
    cv2.putText(frame, f"Masuk: {right_count}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    cv2.putText(frame, f"Keluar: {left_count}", (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

    cv2.imshow("People Counting Left/Right", frame)
    out.write(frame)  # simpan setiap frame ke video
...
print(f"Selesai! Masuk: {right_count}, Keluar: {left_count}")
